/*
 * Gladia Control API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct TranslationDto {
    /// The audio intelligence model succeeded to get a valid output
    #[serde(rename = "success")]
    pub success: bool,
    /// The audio intelligence model returned an empty value
    #[serde(rename = "is_empty")]
    pub is_empty: bool,
    /// Time audio intelligence model took to complete the task
    #[serde(rename = "exec_time")]
    pub exec_time: f64,
    /// `null` if `success` is `true`. Contains the error details of the failed model
    #[serde(rename = "error")]
    pub error: Box<models::AddonErrorDto>,
    /// List of translated transcriptions, one for each `target_languages`
    #[serde(rename = "results")]
    pub results: Vec<models::TranslationResultDto>,
}

impl TranslationDto {
    pub fn new(success: bool, is_empty: bool, exec_time: f64, error: models::AddonErrorDto, results: Vec<models::TranslationResultDto>) -> TranslationDto {
        TranslationDto {
            success,
            is_empty,
            exec_time,
            error: Box::new(error),
            results,
        }
    }
}

