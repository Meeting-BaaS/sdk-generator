 ...
 * Generates Zod schemas from manual TypeScript spec
 * @see specs/soniox-streaming-types.ts
...
const MANUAL_SPEC = path.join(__dirname, "../specs/soniox-streaming-types.ts")
...
/**
 * Generate Zod schema from manual TypeScript spec
 * Since Soniox doesn't have an official AsyncAPI spec, we generate
 * Zod schemas based on the manually extracted TypeScript types
 */
function generateZodSchemas() {
  // Read the manual spec to extract version info
  const specContent = fs.readFileSync(MANUAL_SPEC, "utf-8")
...
async function main() {
  try {
    console.log("📥 Generating Soniox streaming types from manual TypeScript spec...")

    // Check if manual spec exists
    if (!fs.existsSync(MANUAL_SPEC)) {
      console.error(`❌ Manual spec not found at ${MANUAL_SPEC}`)
      console.log("   Please create the manual spec file first")
      process.exit(1)
    }
    ...
    // Generate Zod schema from manual spec
    console.log(`  → Reading ${path.relative(process.cwd(), MANUAL_SPEC)}...`)
    const zodContent = generateZodSchemas()

    fs.writeFileSync(STREAMING_ZOD_OUTPUT, zodContent)
    console.log(`  ✅ Generated ${path.relative(process.cwd(), STREAMING_ZOD_OUTPUT)}`)

    console.log("✅ Successfully generated Soniox streaming types!")
    process.exit(0)
  } catch (error) {
    console.error("❌ Failed to generate Soniox streaming types:", error.message)
