...
function main() {
  console.log("📦 Generating Soniox language constants from OpenAPI spec...")
  ...
  if (!getModelsResponse?.example?.models) {
    console.error("❌ Could not find GetModelsResponse.example.models in spec")
    process.exit(1)
  }
  ...
  if (!firstModel?.languages) {
    console.error("❌ Could not find languages in model example")
    process.exit(1)
  }
