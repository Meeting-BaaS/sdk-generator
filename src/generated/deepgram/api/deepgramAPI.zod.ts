...
export const listenTranscribeQuerySummarizeDefaultTwo = false
...
export const listenTranscribeQueryDetectLanguageDefaultOne = false
...
export const listenTranscribeQueryParams = zod.object({
  ...
  callback_method: zod
    .enum(["POST", "PUT"])
    .default(listenTranscribeQueryCallbackMethodDefault)
  ...
  custom_topic_mode: zod
    ...
    .default(listenTranscribeQueryCustomTopicModeDefault)
    .describe(
  ...
  custom_intent_mode: zod
    .enum(["extended", "strict"])
    .default(listenTranscribeQueryCustomIntentModeDefault)
    .describe(
  ...
  language: zod
    .string()
    .default(listenTranscribeQueryLanguageDefault)
    .describe(
  ...
  redact: zod
    .string()
    .or(zod.array(zod.enum(["pci", "pii", "numbers"])))
    .optional()
  ...
  utt_split: zod
    .number()
    .default(listenTranscribeQueryUttSplitDefault)
...
export const listenTranscribeHeader = zod.object({
  Authorization: zod
    .string()
    .describe("Use `Authorization: Token <API_KEY>`\nExample: `Authorization: Token 12345abcdef`\n")
})

export const listenTranscribeBody = zod
  .object({
    url: zod.string().url()
  })
...
export const listenTranscribeResponse = zod
  .object({
    metadata: zod.object({
...
export const speakGenerateQueryParams = zod.object({
  ...
  callback_method: zod
    .enum(["POST", "PUT"])
    .default(speakGenerateQueryCallbackMethodDefault)
    .describe("HTTP method by which the callback request will be made"),
  mip_opt_out: zod
    .boolean()
    .optional()
  ...
  container: zod
    ...
    .or(
      zod
        .enum(["wav"])
        .describe("Encoding - linear16. Supported container - wav (default), or no container.")
    )
    .or(
      zod
        .enum(["wav"])
        .describe("Encoding - mulaw. Supported container - wav (default), or no container.")
    )
    .or(
      zod
        .enum(["wav"])
        .describe("Encoding - alaw. Supported container - wav (default), or no container.")
    )
    .or(zod.enum(["ogg"]).describe("Encoding - opus. Supported container - ogg (default)."))
    .optional()
    .describe(
  ...
  encoding: zod
    ...
    .or(
      zod
        .enum(["flac"])
        .describe("Encoding - flac. Lossless audio format for high-quality compression.")
    )
    .or(
      zod
        .enum(["mulaw"])
        .describe("Encoding - mulaw. Compressed audio format commonly used in telephony.")
    )
    .or(
      zod
        .enum(["alaw"])
        .describe("Encoding - alaw. Similar to mulaw but used in international telephony.")
    )
    .or(
      zod
        .enum(["mp3"])
        .describe("Encoding - mp3. Popular compressed audio format for music and streaming.")
    )
    .or(
      zod
        .enum(["opus"])
    ...
    .optional()
    .describe("Encoding allows you to specify the expected encoding of your audio output"),
  model: zod
    .enum([
      "aura-asteria-en",
      "aura-luna-en",
    ...
    .default(speakGenerateQueryModelDefault)
    .describe("AI model used to process submitted text"),
  sample_rate: zod
    .enum(["8000", "16000", "24000", "32000", "48000"])
    .describe("Encoding - linear16. Supported sample rates - 8000, 16000, 24000, 32000, 48000 Hz.")
    .or(
      zod
        .enum(["8000", "16000"])
        .describe("Encoding - mulaw. Supported sample rates - 8000, 16000 Hz.")
    )
    .or(
      zod
        .enum(["8000", "16000"])
        .describe("Encoding - alaw. Supported sample rates - 8000, 16000 Hz.")
    )
    .or(
      zod
        .enum(["22050"])
        .describe("Encoding - mp3. Sample rate is fixed and not configurable (22050 Hz).")
    )
    .or(zod.enum(["48000"]).describe("Encoding - opus. Sample rate is fixed at 48000 Hz."))
    .optional()
    .describe(
...
export const readAnalyzeQuerySummarizeDefaultTwo = false
...
export const readAnalyzeQueryParams = zod.object({
  ...
  callback_method: zod
    .enum(["POST", "PUT"])
    .default(readAnalyzeQueryCallbackMethodDefault)
    .describe("HTTP method by which the callback request will be made"),
  sentiment: zod
    .boolean()
    .optional()
  ...
  custom_topic_mode: zod
    .enum(["extended", "strict"])
    .default(readAnalyzeQueryCustomTopicModeDefault)
    .describe(
      "Sets how the model will interpret strings submitted to the `custom_topic` param. When `strict`, the model will only return topics submitted using the `custom_topic` param. When `extended`, the model will return its own detected topics in addition to those submitted using the `custom_topic` param"
    ),
  intents: zod
  ...
  custom_intent_mode: zod
    .enum(["extended", "strict"])
    .default(readAnalyzeQueryCustomIntentModeDefault)
    .describe(
      "Sets how the model will interpret intents submitted to the `custom_intent` param. When `strict`, the model will only return intents submitted using the `custom_intent` param. When `extended`, the model will return its own detected intents in the `custom_intent` param."
    ),
  language: zod
    .string()
    .default(readAnalyzeQueryLanguageDefault)
    .describe(
      "The [BCP-47 language tag](https://tools.ietf.org/html/bcp47) that hints at the primary spoken language. Depending on the Model and API endpoint you choose only certain languages are available"
    )
})
...
export const readAnalyzeBody = zod
  .object({
...
export const readAnalyzeResponse = zod
  .object({
    metadata: zod.object({
      metadata: zod
        .object({
...
export const listProjectRequestsQueryParams = zod.object({
  ...
  limit: zod
    .number()
    .default(listProjectRequestsQueryLimitDefault)
