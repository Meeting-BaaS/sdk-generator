...
export const getRetranscribeBotResponseMock = (overrideResponse: Partial< RetranscribeBot200 > = {}): RetranscribeBot200 => ({success: faker.datatype.boolean(), data: {message: faker.string.alpha(20)}, ...overrideResponse})
...
export const getRetranscribeBotMockHandler = (overrideResponse?: RetranscribeBot200 | ((info: Parameters<Parameters<typeof http.post>[1]>[0]) => Promise<RetranscribeBot200> | RetranscribeBot200)) => {
  return http.post('https://api.meetingbaas.com/v2/bots/:botId/retranscribe', async (info) => {await delay(1000);
  
    return new HttpResponse(JSON.stringify(overrideResponse !== undefined 
            ? (typeof overrideResponse === "function" ? await overrideResponse(info) : overrideResponse) 
            : getRetranscribeBotResponseMock()),
      { status: 200,
        headers: { 'Content-Type': 'application/json' }
      })
  })
