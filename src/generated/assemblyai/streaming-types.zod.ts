...
export const streamingTranscriberParams = zod.object({
  ...
  sessionHeartbeat: zod.boolean().optional().describe("From SDK v3"),
  acknowledgeSilence: zod.boolean().optional().describe("From SDK v3"),
  ...
  speechModel: zod.enum(["universal-streaming-english", "universal-streaming-multilingual", "u3-rt-pro", "u3-rt-pro-beta-1", "whisper-rt", "universal-3-5-pro", "universal-3-6", "universal-3-6-pro", "u3-pro"]).optional().describe("From SDK v3"),
  ...
  maxSpeakers: zod.number().optional().describe("From SDK v3"),
  voiceFocus: zod.enum(["near-field", "far-field"]).optional().describe("From SDK v3"),
  voiceFocusThreshold: zod.number().optional().describe("From SDK v3"),
  continuousPartials: zod.boolean().optional().describe("From SDK v3"),
  interruptionDelay: zod.number().optional().describe("From SDK v3"),
  turnLeftPadMs: zod.number().optional().describe("From SDK v3"),
  customerSupportAudioCapture: zod.boolean().optional().describe("From SDK v3"),
  includePartialTurns: zod.boolean().optional().describe("From SDK v3"),
  redactPii: zod.boolean().optional().describe("From SDK v3"),
  redactPiiPolicies: zod.array(zod.enum(["account_number", "banking_information", "blood_type", "corporate_action", "credit_card_cvv", "credit_card_expiration", "credit_card_number", "date", "date_interval", "date_of_birth", "day", "drivers_license", "drug", "duration", "effect", "email_address", "event", "filename", "financial_metric", "gender", "gender_sexuality", "healthcare_number", "injury", "ip_address", "language", "location", "location_address", "location_address_street", "location_city", "location_coordinate", "location_country", "location_state", "location_zip", "marital_status", "medical_code", "medical_condition", "medical_process", "money_amount", "month", "nationality", "number_sequence", "occupation", "organization", "organization_id", "organization_medical_facility", "passport_number", "password", "person_age", "person_name", "phone_number", "physical_attribute", "political_affiliation", "product", "project", "religion", "sexuality", "statistics", "time", "trend", "url", "us_social_security_number", "username", "vehicle_id", "year", "zodiac_sign"])).optional().describe("From SDK v3"),
  redactPiiSub: zod.enum(["hash", "entity_name"]).optional().describe("From SDK v3"),
  mode: zod.enum(["max_accuracy", "min_latency", "balanced"]).optional().describe("From SDK v3"),
...
export const streamingUpdateConfigParams = zod.object({
  ...
  session_heartbeat: zod.boolean().optional().describe("From SDK v3"),
  acknowledge_silence: zod.boolean().optional().describe("From SDK v3"),
