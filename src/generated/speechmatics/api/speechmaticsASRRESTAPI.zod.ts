...
export const PostJobsBody = zod.object({
  ...
  "data_file": zod.instanceof(File).optional()
})

export const PostJobsResponse = zod.object({
  "id": zod.string().describe('The unique ID assigned to the job. Keep a record of this for later retrieval of your completed job.')
})
...
export const GetJobsQueryParams = zod.object({
  "created_before": zod.iso.datetime({"offset":true}).optional().describe('UTC Timestamp cursor for paginating request response. Filters jobs based on creation time to the nearest millisecond. Accepts up to nanosecond precision, truncating to millisecond precision. By default, the response will start with the most recent job.'),
  "limit": zod.number().min(1).max(getJobsQueryLimitMax).optional().describe('Limit on the number of items returned. By default, no limit is applied.'),
...
export const GetJobsResponse = zod.object({
  ...
  "created_at": zod.iso.datetime({"offset":true}).describe('The UTC date time the job was created.'),
  ...
  "model": zod.enum(['standard', 'enhanced', 'melia-1']).optional().describe('Specific model to use in transcription (previously called operating point).'),
  "operating_point": zod.enum(['standard', 'enhanced', 'melia-1']).optional().describe('\*\*Deprecated\*\*: Use `model` instead. `operating_point` is left only for backward compatibility.'),
  ...
  "audio_filtering_config": zod.object({
  "volume_threshold": zod.number().min(getJobsResponseJobsItemConfigTranscriptionConfigAudioFilteringConfigVolumeThresholdMin).max(getJobsResponseJobsItemConfigTranscriptionConfigAudioFilteringConfigVolumeThresholdMax).optional().describe('Controls the lower limit of audio volume at which speech and audio events will be transcribed. If the volume limit is very low, then most sound will be passed to the speech recognition engine. Higher numbers will cut out increasing amounts of sound.')
  ...
  "from": zod.string(),
  "to": zod.string()
})).optional().describe('An array of objects defining custom replacements. Each replacement contains a pair of strings: the text to find `from` and the text to replace it with `to`.')
  ...
  "contents": zod.array(zod.enum(['jobinfo', 'transcript', 'transcript.json-v2', 'transcript.txt', 'transcript.srt', 'data', 'text'])).optional().describe('Specifies a list of items to be attached to the notification message. When multiple items are requested, they are included as named file attachments.'),
...
export const GetJobsJobidResponse = zod.object({
  ...
  "max_delay_mode": zod.enum(['fixed', 'flexible']).optional().describe('Whether or not to enable flexible endpointing and allow the entity to continue to be spoken.'),
  "audio_filtering_config": zod.object({
  "volume_threshold": zod.number().min(getJobsJobidResponseJobConfigTranscriptionConfigAudioFilteringConfigVolumeThresholdMin).max(getJobsJobidResponseJobConfigTranscriptionConfigAudioFilteringConfigVolumeThresholdMax).optional().describe('Controls the lower limit of audio volume at which speech and audio events will be transcribed. If the volume limit is very low, then most sound will be passed to the speech recognition engine. Higher numbers will cut out increasing amounts of sound.')
}).optional().describe('Configuration for limiting the transcription of quiet audio.'),
  "transcript_filtering_config": zod.object({
  "remove_disfluencies": zod.boolean().optional().describe('If true, words that are identified as disfluencies will be removed from the transcript. If false (default), they are tagged in the transcript as \'disfluency\'.'),
  "replacements": zod.array(zod.object({
  ...
  "summarization_config": zod.object({
  "content_type": zod.enum(['auto', 'informative', 'conversational']).optional().describe('Choose from three options:\n- `conversational` - Best suited for dialogues involving multiple participants, such as calls, meetings or discussions. It focuses on summarizing key points of the conversation.\n- `informative` - Recommended for more structured information delivered by one or more people, making it ideal for videos, podcasts, lectures, and presentations.\n- `auto` - Automatically selects the most appropriate content type based on an analysis of the transcript.\n'),
  "summary_length": zod.enum(['brief', 'detailed']).default(getJobsJobidResponseJobConfigSummarizationConfigSummaryLengthDefault).describe('Determines the depth of the summary:\n- `brief` - Provides a succinct summary, condensing the content into just a few sentences.\n- `detailed` - Provide a longer, structured summary. For _conversational_ content, it includes key topics and a summary of the entire conversation. For _informative_ content, it logically divides the audio into sections and provides a summary for each.'),
  "summary_type": zod.enum(['paragraphs', 'bullets']).optional()
}).optional().describe('Configuration options for summarization.'),
  "sentiment_analysis_config": zod.looseObject({
...
export const DeleteJobsJobidResponse = zod.object({
  ...
  "max_delay_mode": zod.enum(['fixed', 'flexible']).optional().describe('Whether or not to enable flexible endpointing and allow the entity to continue to be spoken.'),
  "audio_filtering_config": zod.object({
  "volume_threshold": zod.number().min(deleteJobsJobidResponseJobConfigTranscriptionConfigAudioFilteringConfigVolumeThresholdMin).max(deleteJobsJobidResponseJobConfigTranscriptionConfigAudioFilteringConfigVolumeThresholdMax).optional().describe('Controls the lower limit of audio volume at which speech and audio events will be transcribed. If the volume limit is very low, then most sound will be passed to the speech recognition engine. Higher numbers will cut out increasing amounts of sound.')
}).optional().describe('Configuration for limiting the transcription of quiet audio.'),
  "transcript_filtering_config": zod.object({
  "remove_disfluencies": zod.boolean().optional().describe('If true, words that are identified as disfluencies will be removed from the transcript. If false (default), they are tagged in the transcript as \'disfluency\'.'),
  "replacements": zod.array(zod.object({
  ...
  "summarization_config": zod.object({
  "content_type": zod.enum(['auto', 'informative', 'conversational']).optional().describe('Choose from three options:\n- `conversational` - Best suited for dialogues involving multiple participants, such as calls, meetings or discussions. It focuses on summarizing key points of the conversation.\n- `informative` - Recommended for more structured information delivered by one or more people, making it ideal for videos, podcasts, lectures, and presentations.\n- `auto` - Automatically selects the most appropriate content type based on an analysis of the transcript.\n'),
  "summary_length": zod.enum(['brief', 'detailed']).default(deleteJobsJobidResponseJobConfigSummarizationConfigSummaryLengthDefault).describe('Determines the depth of the summary:\n- `brief` - Provides a succinct summary, condensing the content into just a few sentences.\n- `detailed` - Provide a longer, structured summary. For _conversational_ content, it includes key topics and a summary of the entire conversation. For _informative_ content, it logically divides the audio into sections and provides a summary for each.'),
  "summary_type": zod.enum(['paragraphs', 'bullets']).optional()
}).optional().describe('Configuration options for summarization.'),
  "sentiment_analysis_config": zod.looseObject({
...
export const GetJobsJobidTranscriptResponse = zod.object({
  ...
  "metadata": zod.object({
  "created_at": zod.iso.datetime({"offset":true}).describe('The UTC date time the transcription output was created.'),
  "type": zod.enum(['transcription']),
  "transcription_config": zod.object({
  "language": zod.string().describe('Language model to process the audio input, normally specified as an ISO language code'),
  "domain": zod.string().optional().describe('Request a specialized model based on \'language\' but optimized for a particular field, e.g. \'finance\' or \'medical\'.'),
  ...
  "max_delay_mode": zod.enum(['fixed', 'flexible']).optional().describe('Whether or not to enable flexible endpointing and allow the entity to continue to be spoken.'),
  "audio_filtering_config": zod.object({
  "volume_threshold": zod.number().min(getJobsJobidTranscriptResponseMetadataTranscriptionConfigAudioFilteringConfigVolumeThresholdMin).max(getJobsJobidTranscriptResponseMetadataTranscriptionConfigAudioFilteringConfigVolumeThresholdMax).optional().describe('Controls the lower limit of audio volume at which speech and audio events will be transcribed. If the volume limit is very low, then most sound will be passed to the speech recognition engine. Higher numbers will cut out increasing amounts of sound.')
}).optional().describe('Configuration for limiting the transcription of quiet audio.'),
  "transcript_filtering_config": zod.object({
  "remove_disfluencies": zod.boolean().optional().describe('If true, words that are identified as disfluencies will be removed from the transcript. If false (default), they are tagged in the transcript as \'disfluency\'.'),
  "replacements": zod.array(zod.object({
...
}).optional().describe('Result of the language identification of the audio, configured using `language_identification_config`, or setting the transcription language to `auto`.'),
  ...
  "text": zod.string().optional().describe('Represents the transcript of the analysed segment'),
  "sentiment": zod.string().optional().describe('The assigned sentiment to the segment, which can be positive, neutral or negative'),
  "start_time": zod.number().optional().describe('The timestamp corresponding to the beginning of the transcription segment'),
  "end_time": zod.number().optional().describe('The timestamp corresponding to the end of the transcription segment'),
  "speaker": zod.string().optional().describe('The speaker label for the segment, if speaker diarization is enabled'),
  "channel": zod.string().optional().describe('The channel label for the segment, if channel diarization is enabled'),
  "confidence": zod.number().optional().describe('A confidence score in the range of 0-1,\nindicating the model\'s certainty in the predicted sentiment,\n')
}).describe('A an object that holds overall sentiment information, and per-speaker and per-channel sentiment data.')).optional().describe('An array of objects that represent a segment of text and its associated sentiment.'),
  "summary": zod.object({
  "overall": zod.object({
  "positive_count": zod.number().optional(),
  "negative_count": zod.number().optional(),
  "neutral_count": zod.number().optional()
}).optional().describe('Summary of overall sentiment data.'),
  "speakers": zod.array(zod.object({
  "speaker": zod.string().optional(),
  "positive_count": zod.number().optional(),
  "negative_count": zod.number().optional(),
...
}).optional().describe('An object that holds overall sentiment information, and per-speaker and per-channel sentiment data.')
...
}).describe('An object that holds topic information for a single segment.')).optional().describe('An array of objects that represent a segment of text and its associated topic information.'),
  "summary": zod.object({
  "overall": zod.record(zod.string(), zod.number()).optional().describe('Summary of overall topic detection results.')
}).optional().describe('An object that holds overall information on the topics detected.')
  ...
  "summary": zod.string().optional().describe('An auto-generated paragraph-style, short summary of the chapter'),
  "start_time": zod.number().optional().describe('The start time of the chapter in the audio file'),
  "end_time": zod.number().optional()
...
}).describe('Summary statistics for this audio event type')).optional().describe('Overall summary on all channels'),
...
export const GetUsageQueryParams = zod.object({
  "since": zod.iso.date().optional().describe('Include usage after the given date (inclusive). This is a [ISO-8601](https:\/\/en.wikipedia.org\/wiki\/ISO_8601) calendar date format: `YYYY-MM-DD`.'),
  "until": zod.iso.date().optional().describe('Include usage before the given date (inclusive). This is a [ISO-8601](https:\/\/en.wikipedia.org\/wiki\/ISO_8601) calendar date format: `YYYY-MM-DD`.')
})
