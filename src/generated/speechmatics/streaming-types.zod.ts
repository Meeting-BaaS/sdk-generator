...
export const speechmaticsTranscriptionConfigSchema = zod.object({
  language: zod.string().describe("Language model to process the audio input, normally specified as an ISO language code. The value must be consistent with the language code used in the API endpoint URL."),
  ...
  output_locale: zod.unknown() /* TODO: resolve OutputLocale */.optional(),
  additional_vocab: zod.unknown() /* TODO: resolve VocabList */.optional(),
  diarization: zod.unknown() /* TODO: resolve DiarizationConfig */.optional(),
  max_delay: zod.number().min(0.7).max(4).optional().describe("This is the delay in seconds between the end of a spoken word and returning the Final transcript results. See [Latency](https://docs.speechmatics.com/speech-to-text/realtime/output#latency) for more details"),
  max_delay_mode: zod.enum(["flexible", "fixed"]).optional(),
  speaker_diarization_config: zod.unknown() /* TODO: resolve SpeakerDiarizationConfig */.optional(),
  audio_filtering_config: zod.unknown() /* TODO: resolve AudioFilteringConfig */.optional(),
...
export const speechmaticsMidSessionConfigSchema = zod.object({
  language: zod.string().optional().describe("Language model to process the audio input, normally specified as an ISO language code. The value must be consistent with the language code used in the API endpoint URL."),
  max_delay: zod.number().min(0.7).max(4).optional().describe("This is the delay in seconds between the end of a spoken word and returning the Final transcript results. See [Latency](https://docs.speechmatics.com/speech-to-text/realtime/output#latency) for more details"),
  max_delay_mode: zod.enum(["flexible", "fixed"]).optional(),
  audio_filtering_config: zod.unknown() /* TODO: resolve AudioFilteringConfig */.optional(),
  enable_partials: zod.boolean().optional().describe("Whether or not to send Partials (i.e. `AddPartialTranslation` messages) as well as Finals (i.e. `AddTranslation` messages) See [Partial transcripts](https://docs.speechmatics.com/speech-to-text/realtime/output#partial-transcripts)."),
...
export const streamingTranscriberParams = zod.object({
  encoding: speechmaticsAudioEncodingSchema.optional()
