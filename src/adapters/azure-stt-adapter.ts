...
export class AzureSTTAdapter extends BaseAdapter {
	...
	initialize(config: ProviderConfig & { region?: string }): void {
		...
		this.baseUrl =
			config.baseUrl ||
			`https://${this.region}.api.cognitive.microsoft.com/speechtotext/v3.1`
		...
		if (audio.type !== "url") {
			return {
				...
				error: {
					code: "INVALID_INPUT",
					message:
						"Azure Speech-to-Text batch transcription only supports URL input",
				},
			}
		}
	 ...
	 */
	private buildTranscriptionProperties(
		options?: TranscribeOptions,
	...
	private normalizeResponse(
		...
		const fullText =
			combinedPhrases
				.map((phrase: any) => phrase.display || phrase.lexical)
				.join(" ") || ""
		...
		return {
			...
			data: {
				...
				duration: transcriptionData.duration
					? transcriptionData.duration / 10000000
					: undefined,
				speakers,
